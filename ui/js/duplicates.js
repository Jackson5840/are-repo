...
var baseurl = 'https://neuromorpho.org/ingestapi/';
...
$('#example').on('click', 'a.editor_ignore', function (e) {
    ...
    if (confirm("Are you sure you wish to ignore this potential duplicate?")) {
        var rdata = table.row( $(this).parents('tr') ).data();
        fetch(baseurl +'/handleduplicate/' + rdata['dupname'] + '/ignore',{ method: 'get', mode: 'cors' })
            .then(
                function(response) {
...
$('#example').on('click', 'a.editor_remove', function (e) {
    ...
    if (confirm("Are you sure you wish to delete this neuron from main and review DB?")) {
        var rdata = table.row( $(this).parents('tr') ).data();
        fetch(baseurl +'/handleduplicate/' + rdata['dupname'] + '/delete',{ method: 'get', mode: 'cors' })
            .then(
                function(response) {
                if (response.status !== 200) {
                    console.log('Looks like there was a problem. Status Code: ' +
...
$(document).ready(function() {
    table = $('#example').DataTable( {
        "ajax": "../assets/ajax/duplicates2023.json",
        "columns": [
            {
            ...
            { "data": "level"},
            {
                data: null,
                className: "center",
                defaultContent: '<a href="" class="editor_ignore">Ignore</a> / <a href="" class="editor_remove">Delete</a>'
            }
            //{ "data": "d_level"}
//            { "data": "d_imageurl" },
//            { "data": "d_neuronurl" },
